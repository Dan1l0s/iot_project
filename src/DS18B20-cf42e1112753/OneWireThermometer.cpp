...
OneWireThermometer::OneWireThermometer(bool crcOn, bool useAddr, bool parasitic, PinName pin, int device_id) : useCRC(crcOn), useAddress(useAddr), useParasiticPower(parasitic),
...
float OneWireThermometer::readTemperature()
{
    ...
    if (useParasiticPower)
    {
        ...
        thread_sleep_for(CONVERSION_TIME[resolution]);
    }
    else
    {
        // TODO
